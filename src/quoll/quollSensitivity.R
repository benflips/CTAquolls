...
#sTime<-system.time({
n<-1 # number of iterations (test) for PVA
...
pr<-array(NA, dim = c(length(h2), length(CTAh), length(CTAe))) #extinction prob
...
for (ee in 1:length(CTAe)){
  for (cc in 1:length(CTAh)){
    for (hh in 1:length(h2)){
      ...
      close(pb)
      pr[hh, cc, ee]<-length(e[e==TRUE])/n
      pArr[hh, cc, ee]<-mean(p, na.rm = TRUE)
      cArr[hh, cc, ee]<-mean(c, na.rm = TRUE)
    }
  }
}

save(pr, pArr, cArr, CTAe, CTAh, h2, file = "out/extProbWithTransmission.RData")
