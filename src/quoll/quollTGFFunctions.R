# Heavily modified from IBM reported in (Kelly & Phillips 2018)
# Mods by Ben Phillips
# For publication in 
...
initInds<-function(K, nP, nI, nN, f, Ve, h2, recipient=TRUE){
  ...
  A<-rep(1, n) #age 
  C<-rep(0, n) # CTA training status, initially set to zero
  ...
  cbind(S, A, C, allelesP, allelesN, allelesI)	
}

# calculates attack phenotype given alleles, effect size, Ve, and CTA status
  # effect size needs to be scaled carefully against Ve and mu to generate initial fitness.
aPhen<-function(pop, eSize, Ve, sel, CTA=TRUE){
  gtypes<-pop[,grepl("a", colnames(pop)), drop=FALSE]
  gtypes<-apply(gtypes, 1, sum)*eSize
  ptypes<-rnorm(length(gtypes), mean=gtypes, sd=sqrt(Ve)) #phenotypes
  ptypes<-ptypes>sel #thresholded attack or not, based on genetics
  if (CTA) ptypes<-(pop[,"C"]==1 | ptypes) # attack or not: genetics + CTA.  Not attack == 1
  ptypes
}
...
repro<-function(pop, K, nP, nN, nI, s0, lLoci, gCols, recombRate, CTAh){
  ...
  A<-rep(0, nOff)
  C<-females[offRows, "C"]*rbinom(n = nOff, size = 1, prob = CTAh)
  rbind(pop, cbind(S, A, C, alleles))
}

# Survival and age (based on age and phenotype)
surv<-function(pop, fsurv1, fsurv2, msurv1, selection, eSize, Ve, s, outbreeding, nl, h2, alpha, CTA){
...
TGF<-function(demPars, nP, nN, nI, W0, f0, h2=0.3, Ve=1, s0=1, K, burnIn=10, selGens=50, outbreeding=FALSE, nl, H2, alpha=1, f1, recombRate, CTAh, CTAe){
  # global parameters
  ...
  iterate<-function(start, finish, collect=TRUE, selection=FALSE, outbreeding){
    for (gg in start:finish){
      pop<<-repro(pop, K, nP, nN, nI, s0, lLoci, gCols, recombRate, CTAh)
      ...
      if (collect){
        ...
        if (nr<=1) {phenGen<<-c(phenGen, NA); phenCTA<<-c(phenCTA, NA); break}
        phenGen<<-c(phenGen, mean(aPhen(pop, eSize=selPars$eSize, Ve, sel=selPars$s, CTA = FALSE))) # genetic-based phenotype
        phenCTA<<-c(phenCTA, mean(pop[,"C"]))
      }
    }
  }
  # burn in
  ...
  # implement CTA
  trainSS<-aPhen(pop, eSize=selPars$eSize, Ve=Ve, sel=selPars$s)==0
  pop[trainSS,"C"]<-rbinom(n = sum(trainSS), size = 1, prob = CTAe)
  # run over selGens
  iterate(1, selGens, collect = TRUE, selection = TRUE, outbreeding)
  
  list(n=n, phenGen=phenGen, phenCTA=phenCTA, selPars = selPars)
}
